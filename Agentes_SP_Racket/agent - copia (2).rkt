  ...
  (class object%
        ...
        (cond
          ...
          [(if colision
               ...
               (begin
                 ...
                 (printf "nueva posicion: ~a\n" nuevaPosicion)
                 (printf "Agente ~a: Se movio hacia delante.\n" id)
               )
             )]
          )))
    ...
    (define/public (mover-hacia-objeto dc object objects)
      (let* ([pos (unbox posicion)]
             [posObj (send object obtener-posicion)] 
             ...
             ; Verificar si colisiona con algo
             [colision (findf (λ (obj) (colision? nuevaPosicion obj)) objects)]

             ;; Detectar colisiones con objetos tipo "machine"
             [colisionMachine
              (findf (λ (obj) (and (colision? nuevaPosicion obj)
                                   (string=? "Maquina" (send obj obtener-tipo))))
                     objects)])
        ...
        (cond
          ...
          ;; Colisiona con algo no deseado
          [(and (not (empty? colision))
                (colision? nuevaPosicion object)
                (is-in-recharge-zone nuevaPosicion object))
           (begin
             (set-box! orientacion (random 120 270))
             (send this mover-hacia-objeto dc object objects))]

          ;; Colisión con objeto tipo "machine"
          [colisionMachine
           (begin
             (set-box! orientacion (random 120 270)) ; Cambia a una orientación aleatoria
             (send this mover-hacia-objeto dc object objects))]
    ...
    (define/public (gestionar-eventos dc objects)
      (let ([energiaActual (unbox energia)]
        ...
        (cond
          ...
          [(and (is-a? estadoAgente sin-evento%) (< energiaActual umbral-hambre))
          ...
          [(is-a? estadoAgente evento-activo%)
           (printf "Agente ~a: Continuando con hambre.\n" id)
           (send this realizar-evento-hambre dc objects)
          ]
          [else
      ...
      (let* ([pos (unbox posicion)]
        ...
        (cond
          ...
          [(= (length registrados) (length objects))
           (printf "Agente ~a: Ya tiene todos los objetos registrados." id)
           (send this mover-adelante dc objects)
           ]
